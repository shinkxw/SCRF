...
class C数据采集器
  ...
  def 进行采集(限制时间 = 60, 输出采集信息 = false, 记录耗时 = false)
    ...
    @采集方式, 时间 = 记录耗时 ? :单次计时采集 : :单次不计时采集, R进程.单调时钟时间
    ...
    输出 "数据采集结束, 共耗时: #{R进程.单调时钟时间 - 时间}" if @输出采集信息
    return @采集结果.所有值
  rescue E超时异常 => 超时异常
    引发异常('数据采集异常','数据采集超时')
  end
  ...
  def 单次计时采集(参数数组)
    时间, 结果 = R进程.单调时钟时间, @块[*参数数组]
    @采集结果[参数数组] = @结果类.新建(*参数数组, 结果, R进程.单调时钟时间 - 时间)
  end
...
描述 C数据采集器 do
  添加测试 '多参数数据采集测试' do
    数据集 = C数据采集器.新建([:x, :y, :z], [1..5, 11..15, 51..55]){|x,y,z| x * y - z}.进行采集
    数据集.每个{|数据| 期望(数据.x * 数据.y - 数据.z).等于 数据.结果} 
  end
  
  添加测试 '耗时信息采集测试' do
    数据集 = C数据采集器.新建([:时间], [2..4]){|时间| 暂停(时间 / 100.0)}.进行采集(1, false, true)
    数据集.每个{|数据| 期望(数据.时间 / 100.0).接近 数据.消耗时间, 0.015} 
  end
  
  添加测试 '采集单次超时测试' do
    期望(proc {C数据采集器.新建([:x], [1..10]){|x| 暂停(0.2)}.进行采集(0.01)}).应引发 E异常
  end
  
  添加测试 '采集多次超时测试' do
    消耗时间 = 耗时 {C数据采集器.新建([:x], [1..500]){|x| 暂停(0.001)}.进行采集(0.3)}
    期望(消耗时间).接近 0.3, 0.02
  end
end